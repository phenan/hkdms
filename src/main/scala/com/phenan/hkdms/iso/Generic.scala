...
given productGeneric [T <: Product, U <: Tuple] (using mirror: Mirror.ProductOf[T], proof: mirror.MirroredElemTypes =:= U): Iso[T, U] = {
  val to = (value: T) => mirror.fromProduct(value).asInstanceOf[U]
  val from = (underlying: U) => Tuple.fromProduct(underlying).asInstanceOf[T]
  Iso(to, from)
}

given unionGeneric [T, U <: Tuple] (using mirror: Mirror.SumOf[T], proof: mirror.MirroredElemTypes =:= U): Iso[T, Tuple.Union[U]] = {
